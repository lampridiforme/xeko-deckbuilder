import { CardBorderColors, CardData, CardType, Pack, Rarity, SpeciesType } from "../components/info-article/info-definitions";
...
const packStringToEnum: Map<string, Pack> = new Map([
...
const rarityStringToEnum: Map<string, Rarity> = new Map([
...
const cardTypeStringToEnum: Map<string, CardType> = new Map([
...
const borderStringToEnum: Map<string, CardBorderColors> = new Map([
...
export const madagascarCardData: Array<CardData> = Object.entries(data).map(([key, value]) => {
    return {
        ...
        borders: {
            left: new Set((value.borders.left as Array<string>).map(borderString => borderStringToEnum.get(borderString))),
            right: new Set((value.borders.right as Array<string>).map(borderString => borderStringToEnum.get(borderString))),
            top: new Set((value.borders.top as Array<string>).map(borderString => borderStringToEnum.get(borderString))),
            bottom: new Set((value.borders.bottom as Array<string>).map(borderString => borderStringToEnum.get(borderString)))
        },
        energy: value.energy,
        points: value.points,
        level: value.level,
...
export const madagascarNavData: NavInfo = {
    deck: 'Madagascar',
    cards: madagascarCardData.map(card => ({name: card.name, id: card.id}))
}

export const dataMap: Map<string, CardData> = new Map(madagascarCardData.map(entry => [entry.id, entry]));